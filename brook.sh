...
Set_port(){
	while true
		...
		read -e -p "(默认: 20309):" bk_port
		[[ -z "${bk_port}" ]] && bk_port="20309"
...
Set_passwd(){
	...
	read -e -p "(默认: 199608300):" bk_passwd
	[[ -z "${bk_passwd}" ]] && bk_passwd="199608300"
	echo && echo "========================"
...
View_brook(){
	...
	echo
	echo -e "${Tip} Brook链接 Brook Tools客户端（https://github.com/txthinking/brook）OR（https://github.com/fxuan2030/brook）。"
	echo
}
brook_link(){
...
View_user_connection_info(){
 ...
 ${Green_font_prefix}2.${Font_color_suffix} 显示 IP+IP归属地 格式" && echo
	read -e -p "(默认: 2):" brook_connection_info
	[[ -z "${brook_connection_info}" ]] && brook_connection_info="2"
